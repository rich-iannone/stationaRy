...
test_that("select_isd_stations can return a station ID", {
  ...
  expect_match(
    cypress_bowl_snowboard_select_1, "[0-9]*?-[0-9]*?")
  
  expect_match(
    cypress_bowl_snowboard_select_2, "[0-9]*?-[0-9]*?")
  ...
  expect_is(
    cypress_bowl_snowboard_select_multiple, "logical")
  
  # Expect a particular message when multiple stations matched
  expect_message(get_isd_stations(
    lower_lat = 49.000,
    upper_lat = 49.500,
    lower_lon = -123.500,
    upper_lon = -123.000) %>%
