...
test_that("error messages are provided in certain situations", {
  ...
  expect_is(
    df_data_report_data_local_test, "data.frame")
  
  # Expect all possible additional data categories to be present
  expect_equal(
    nrow(df_data_report_data_local_test), 85L)
  
  # Expect one record for each additional data category
  expect_true(
    all(df_data_report_data_local_test$total_count == 2))
  ...
  expect_equal(
    nrow(df_data_additional_data_local_test), 2L)
})
