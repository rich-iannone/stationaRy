...
There are lots of stations and we at least need an identifier to access met data. We can examine station metadata using the `get_station_metadata()` function (which has those ID values in the first column). Let's get all of the stations located in Norway.
...
This table can be greatly reduced to isolate the stations of interest. For example, another use of `dplyr::filter()` could be used to get only high-altitude stations (above 1000 meters). 
